...
TEST_CASE("the registry holds all nineteen entries", "[themes][registry]") {
    REQUIRE(theme_registry().size() == 19);
}
...
TEST_CASE("every documented theme is findable by name", "[themes][registry]") {
    for (const char* n : {"pure", "model", "ideal", "mirror", "promotion", "underpromotion", "excelsior",
                          ...
                          "single-piece", "single-piece:white", "single-piece:black", "en-passant", "kniest"})
